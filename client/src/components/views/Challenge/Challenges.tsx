...
export default function Challenges() {
  ...
  const challengesToDisplay = useMemo(() => {
    ...
    if (shouldFilter) {
      filtered = filtered.filter(
        (challenge) => categories[challenge.category!]
      );
    }
    filtered.sort((a, b) => {
  ...
  const CategoryCheck = useCallback(
    (props: { category: CreateChallengeRequestDtoCategoryEnum }) => {
      ...
      return (
        <div className="categoryCheck">
          ...
          />
          <label htmlFor={`category-${category}`} className="formLabel">
            {category} ({categoryCounts.get(category)?.solved || 0}/
            {categoryCounts.get(category)?.total || 0} solved)
          </label>
        </div>
      );
    },
        ...
        <div className="Col-1">
            ...
            <div className="frameBody">
              ...
              <div className="showSolved">
                <div className="formCheck">
                  ...
                  />
                  <label htmlFor="show_solved" className="formLabel">
                    Show Solved ({solvedCount}/{challenges.length} solved)
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="ChallFrame">
            <div className="frameBody">
              ...
              <CategoryCheck
                category={CreateChallengeRequestDtoCategoryEnum.Forensic}
              />
              <CategoryCheck
                category={CreateChallengeRequestDtoCategoryEnum.Misc}
              />
              <CategoryCheck
                category={CreateChallengeRequestDtoCategoryEnum.Pwnable}
              />
              <CategoryCheck
                category={CreateChallengeRequestDtoCategoryEnum.Reversing}
              />
              <CategoryCheck
                category={CreateChallengeRequestDtoCategoryEnum.Web}
              />
            </div>
          </div>
        </div>
